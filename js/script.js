...
$.modal = (options) => {
  ...
  function _createModal(options) {
    ...
    if (header) {
      ...
      header.innerHTML = `<span class="modal-title">${title}</span>
          ${closeBtn?'<button class="modal-close" data-close="true">&times;</button></div>':''}`
  ...
  body.setAttribute("data-content", true)
  ...
  if (footerButtons.length > 0) {
    ...
    footerButtons.forEach(btn => {
      ...
      btn.class.split(' ').forEach(clas=>{
        button.classList.add(clas)
      })
      button.onclick = btn.callback || dummy
...
const sliders = document.querySelectorAll('.swiper123')
if (sliders) {
  sliders.forEach(slider=>{
    if (!slider.classList.contains('swiper-bild')) {
      let slider_items = Array.from(slider.children);
      if (slider_items) {
      ...
      if (slider.classList.contains('swiper_scroll')) {
        let sliderScrol = document.createElement('div');
        sliderScrol.classList.add('swiper-scrollbar');
        slider.appendChild(sliderScrol);
      }
    }
    if (slider.classList.contains('gallery')) {
      //sLider.data('lightGanery1).destroy(true); 
    }
  })
  for (let i = 0; i < sliders.lengt; i++) 
...
if (document.querySelector('.slider__body')) {
  new Swiper('.slider__body', {
    ...
    parallax: true,
    pagination: {
      el: '.main-slider__slider .slider__dots',
      clickable:true,
    },
    navigation: {
      nextEl: '.main-slider__slider .slider__arrow_next',
      prevEl: '.main-slider__slider .slider__arrow_prev',
    },
...
if (document.querySelector('.slider-rooms__body')) {
  new Swiper('.slider-rooms__body', {
    ...
    navigation: {
      nextEl: '.slider-rooms__arrows .slider__arrow_next',
      prevEl: '.slider-rooms__arrows .slider__arrow_prev',
    },
    // And if we need scrollbar
    // scrollbar: {
    //   el: '.swiper-scrollbar',
...
if (document.querySelector('.tips__slider__body')) {
  new Swiper('.tips__slider__body', {
    observer: true,
    observeParents: true,
    slidesPerView: 3,
    spaceBetween: 32,
    ...
    parallax: true,
    pagination: {
      el: '.tips__slider__dots',
      clickable:true,
    },
    navigation: {
      nextEl: '.tips__slider__arrows .slider__arrow_next',
      prevEl: '.tips__slider__arrows .slider__arrow_prev',
    },
    // And if we need scrollbar
    // scrollbar: {
    //   el: '.swiper-scrollbar',
...
const modal = $.modal({
  content: `<h1>It'sworking</h1>
  <p>Далеко-далеко, за словесными горами в стране гласных и согласных живут рыбные тексты. Маленький даль свою запятой свое ipsum, образ пустился текст семантика.  </p>  `,
  footerButtons: [{
      text: 'Ok',
...
function documentActions(e) {
  ...
  if (window.innerWidth > 768 && isMobile.any()) {
    if (el.classList.contains('menu__arrow')) {
      el.closest('.menu__item').classList.toggle('_hover')
    }
    let _hovered = document.querySelectorAll('.menu__item._hover')
    if (!el.closest('.menu__item') && _hovered.length > 0) {
      _hovered.forEach(item => item.classList.remove('_hover'))
    }
  }
  const searchForm = document.querySelector('.search-form')
  if (el.classList.contains('search-form__icon')) {
    searchForm.classList.toggle('active')
  } else if (!el.closest('.search-form') && searchForm.classList.contains('active')) {
    searchForm.classList.remove('active')
  }
  if (el.classList.contains('products__more')) {
    if (getProducts(el)) {
      return false;
    }
    e.preventDefault()
  ...
  if (el.classList.contains('cart-item__delete')) {
    let parent = el.closest('.cart-item')
    let id = parent.dataset.cartPid
    
    if(updateCart(id,false)){
      parent.remove()
    }
    e.preventDefault()
  }

  if (el.classList.contains('cart-header__icon') || el.closest('.cart-header__icon')) {
    if (document.querySelector('.cart-list').children.length > 0) {
      document.querySelector('.cart-header').classList.toggle('active')
    }
    e.preventDefault()
  } else if (!el.closest('.cart-header') && !el.classList.contains('actions-product__button')) {
    document.querySelector('.cart-header').classList.remove('active')
  }
  if (el.classList.contains('actions-product__button')) {
    let productId = el.closest('.item-product').dataset.pid
    if (!el.classList.contains('hold')) {
      el.classList.add('hold')
      el.classList.add('fly')
      animateImg(productId)
      addToCart(productId)
...
if (spoilers.length > 0) {
  const regularSpoilers = Array.from(spoilers).filter(sp => !sp.dataset.spoilers.split(',')[0])
  if (regularSpoilers.length > 0) {
    initSpoilers(regularSpoilers)
  }
  const mediaSpoilers = Array.from(spoilers).filter(sp => sp.dataset.spoilers.split(',')[0])
  if (mediaSpoilers.length > 0) {
    const breakpointsArray = []
    mediaSpoilers.forEach(sp => {
      const params = sp.dataset.spoilers
      const breakpoint = {}
      const [value, type] = params.split(",")
      breakpoint.value = value
      breakpoint.type = type ? type.trim() : 'max'
      breakpoint.item = sp
      breakpointsArray.push(breakpoint)
    })
    let mdQueries = breakpointsArray.map(item => `(${item.type}-width: ${item.value}px),${item.value},${item.type}`)
    mdQueries = mdQueries.filter((item, index, self) => self.indexOf(item) === index)

    mdQueries.forEach(brp => {
      const param = brp.split(',')
      const breakpoint = param[1]
      const mdType = param[2]
      const matchMedia = window.matchMedia(param[0])
      const spoilerArray = breakpointsArray.filter(br => br.type === mdType && br.value === breakpoint)
  ...
  function initSpoilers(spoilerArray, matchMedia = false) {
    spoilerArray.forEach(sp => {
      sp = matchMedia ? sp.item : sp
      if (matchMedia.matches || !matchMedia) {
        sp.classList.add('init')
        initSpoilerBody(sp)
        sp.addEventListener('click', setSpoilerAction)
      } else {
        sp.classList.remove('init')
        initSpoilerBody(sp, false)
        sp.removeEventListener('click', setSpoilerAction)
      }
    })
  }

  function initSpoilerBody(spoilerBlock, hideSpoilerBody = true) {
    const spHeaders = spoilerBlock.querySelectorAll('[data-spoiler]')
    if (spHeaders.length > 0) {
      spHeaders.forEach(spHeader => {
        if (hideSpoilerBody) {
          spHeader.removeAttribute('tabindex')
          if (!spHeader.classList.contains('active')) {
            spHeader.nextElementSibling.hidden = true
          }
        } else {
          spHeader.setAttribute('tabindex', '-1')
          spHeader.nextElementSibling.hidden = false
        }
      })
    }
  }
  ...
  function setSpoilerAction(e) {
    ...
    if (el.hasAttribute('data-spoiler') || el.closest('[data-spoiler]')) {
      const spTitle = el.hasAttribute('data-spoiler') ? el : el.closest('[data-spoiler]')
      const spoilersBlock = spTitle.closest('[data-spoilers]')
      const oneSp = spoilersBlock.hasAttribute('data-one-spoiler')
      if (!spoilersBlock.querySelectorAll('.slide').length) {
        if (oneSp && !spTitle.classList.contains('active')) {
          hideSpoilersBody(spoilersBlock)
        }
        spTitle.classList.toggle('active')
        slideToggle(spTitle.nextElementSibling, 500)
      }
...
function _open(el, time = 500, display = 'block') {
  ...
  data.products.forEach(item => {
    ...
    if (item.labels.length > 0) {
      item.labels.forEach(label => {
        labels += ` <div class="item-product__label item-product__label_${label.type}">${label.value}</div>`
      })
    }

    let oldPrice = ''
    if (item.priceOld) {
      oldPrice = `<div class="item-product__price item-product__price_old">Rp ${item.priceOld}</div>`
    }
    let tpl = ''
    tpl += `            <article data-pid="${item.id}" class="products__item item-product">
              ...
              <div class="item-product__body">
                ...
                <div class="item-product__actions actions-product">
                  <div class="actions-product__body">
                    <a href="${item.url}" class="actions-product__button btn btn_white">Add to cart</a>
                    <a href="${item.shareUrl}" class="actions-product__link icon-share">Share</a>
                    <a href="${item.likeUrl}" class="actions-product__link icon-heart">Like</a>
                  </div>
                </div>
              </div>
            </article>`
...
function animateImg(id) {
  ...
  let flyImgHeight = productImg.offsetHeight
  let flyImgWidth = productImg.offsetWidth
  let flyImgTop = productImg.getBoundingClientRect().top
  let flyImgLeft = productImg.getBoundingClientRect().left

  flyImg.setAttribute('class', 'flyImg _ibg')
  ...
  flyImg.addEventListener('transitionend', (e) => {
    if (e.propertyName == 'top') {
      flyImg.remove()
      updateCart(id)
      product.querySelector('.actions-product__button').classList.remove('fly')
    }

  })
}

function updateCart(id, add = true) {
  const cart = document.querySelector('.cart-header')
  const cartIcon = cart.querySelector('.cart-header__icon')
  const cartList = cart.querySelector('.cart-list')
  const cartQty = cartIcon.querySelector('span')
  const cartItem = document.querySelector(`[data-cart-pid="${id}"]`)
  ...
  if (add) {
    ...
    if (!cartItem) {
      let product = document.querySelector(`[data-pid="${id}"]`)

      let tpl = `    <li class="cart-list__item cart-item" data-cart-pid="${id}">
      <a href="" class="cart-item__img _ibg">${product.querySelector('.item-product__image').innerHTML}</a>
      <div class="cart-item__body">
        <a href="" class="cart-item__title">${product.querySelector('.item-product__title').innerHTML}</a>
        <div class="cart-item__qty">x<span>1</span></div>
        <button class="cart-item__delete delete">delete</button>
      </div>
    </li>`
      cartList.insertAdjacentHTML('beforeend', tpl)
    } else {
      let cartItemQty = cartItem.querySelector('.cart-item__qty span')
      cartItemQty.innerHTML = ++cartItemQty.innerHTML
    }
    
  }else{
    let cartItemQty = cartItem.querySelector('.cart-item__qty span')
    cartQty.innerHTML =  cartQty.innerHTML - cartItemQty.innerHTML
...
if (gallery && !isMobile.any()) {
  let shareItems = document.querySelector('.share__items');
  let colunms = document.querySelectorAll('.share__column');

  const speed = parseFloat(gallery.data?.speed) || '0.05'
  let posX = 0,
  coordXpercent = 0

  function setMouseGallryStyle() {
    let itemsWidth = 0;
    colunms.forEach(item =>{
      itemsWidth+= item.offsetWidth
    })

    const diff = itemsWidth - shareItems.offsetWidth
    ...
    shareItems.style.cssText = `transform: translate3d(${-pos}px,0,0);`

    if (Math.abs(distX)>0) {
      requestAnimationFrame(setMouseGallryStyle)
    }else{
      shareItems.classList.remove('init')
    }
  }
  gallery.addEventListener('mousemove',e=>{
    let galleryWidth = gallery.offsetWidth

    let coordX = e.pageX - galleryWidth/2

    coordXpercent = coordX/galleryWidth*200
